...
- **Hardware tier depends on GENCAST_ENABLED:**
  - `GENCAST_ENABLED=false` (deterministic-only): **1× A100 80GB** tier (~$2.50/hr) is sufficient. Runs GraphCast 0.25° in ~10-12 min wall-clock, ~$0.40-0.60 per weekly run.
  - `GENCAST_ENABLED=true` (full hybrid): **4× A100 80GB** tier (~$10/hr) required — GenCast 1.0° ensemble is `xarray_jax.pmap`'d across 4 devices (3 pmap batches × 4 members = 12). Host RAM ~568 GB for JIT compile. ~20-25 min wall-clock, ~$3-4 per weekly run.
  - Production default: `GENCAST_ENABLED=false` + 1× A100 (cost-optimised). Flip to `true` + 4× A100 when probabilistic rainfall is needed.
  - Fallback chain kicks in if NWP stack unavailable: NeuralGCM on L4 → Open-Meteo without GPU.
- **NWP init source:** `NWP_INIT_SOURCE=gfs` uses NOAA GFS 0.25° (byte-range fetched from `s3://noaa-gfs-bdp-pds`, ~3h publication lag, ~70 MB per cycle step). Unset or `era5t` uses the ARCO ERA5 Zarr (free, ~5-day lag — retroactive "forecasts"). GFS is the current production default; the code under `src/init_sources/` handles the fetch, TOA reconstruction, and ERA5-shape adapter. Set `GFS_STATIC_CACHE` to the cached netCDF path (auto-built on first run) and `GFS_CACHE_DIR` for raw GRIBs (`/tmp/...` is fine — ephemeral, re-downloaded per wake-up).
- **Env vars (Space secrets — set these before pushing):** `DATABASE_URL`, `ANTHROPIC_API_KEY`, `TOMORROW_IO_API_KEY`, `JWT_SECRET_KEY` (required if `ENV=production`), `WEBHOOK_SECRET` (only if webhook receiver is wired in), optional `ALLOWED_ORIGINS`, `NWP_INIT_SOURCE` (default `gfs`), `GENCAST_ENABLED` (default `false` on production), `GFS_STATIC_CACHE`, `GFS_CACHE_DIR`
