...
_SYSTEM_PROMPT = (
    ...
    "single most important action for this farmer this week. Generate both an "
    "English SMS and a local-language SMS — both must stay under 160 characters. "
    "Return all four values (English advisory, local advisory, English SMS, "
    "local SMS) via the emit_personalized_advisory tool."
)
...
class PersonalizedAdvisoryProvider:
    ...
    async def personalize(
        ...
        tools = [{
            "name": "emit_personalized_advisory",
            "description": "Emit the personalized advisory in English and local language, with SMS versions for both",
            "input_schema": {
                "type": "object",
                "properties": {
                    "advisory_en": {
                        "type": "string",
                        "description": "3-5 sentence personalized advisory in plain English, naming the farmer's crops and giving 1-2 concrete actions",
                    },
                    "advisory_local": {
                        "type": "string",
                        "description": f"The same advisory in {lang_name}, 3-5 sentences, same length as the English version",
                    },
                    "sms_en": {
                        "type": "string",
                        "description": "English SMS version, 160 characters maximum, plain text, no markdown, calling out the single most important action",
                    },
                    "sms_local": {
                        "type": "string",
                        "description": f"SMS version in {lang_name}, 160 characters maximum, plain text, no markdown",
                    },
                },
                "required": ["advisory_en", "advisory_local", "sms_en", "sms_local"],
            },
        }]
        ...
        msg = await client.messages.create(
            ...
            tools=tools,
            tool_choice={"type": "tool", "name": "emit_personalized_advisory"},
        )
        ...
        for block in (msg.content or []):
            if getattr(block, "type", None) == "tool_use" and getattr(block, "name", None) == "emit_personalized_advisory":
