...
class PersonalizedAdvisoryProvider:
    ...
    async def personalize(
        ...
        advisory_en, advisory_local = _parse_dual_output(text)
...
def _parse_dual_output(text: str) -> tuple[str, str]:
    """Split the model's ENGLISH:/LOCAL: structured output into two strings.

    Falls back to (text, "") if the model ignored the format — which rarely
    happens with Haiku 4.5 on a task this structured, but we don't want to
    error out of the whole pipeline on a parse miss.
    """
    en_match = re.search(r"ENGLISH:\s*(.*?)\s*(?:LOCAL:|$)", text, re.DOTALL | re.IGNORECASE)
    local_match = re.search(r"LOCAL:\s*(.*?)\s*$", text, re.DOTALL | re.IGNORECASE)
