...
const DEGRADATION_TIERS = [
  ...
  { tier: 'Tier 2', name: 'Open-Meteo + MOS', sub: 'Numerical weather prediction', color: '#2a9d8f',
    desc: 'GFS/ECMWF forecasts via Open-Meteo API, corrected with the same local MOS model.' },
...
export default function Forecasts() {
  ...
  const mosCount = allForecasts.filter(f => (f.model ?? '').includes('mos')).length
  const mosPct = totalForecasts > 0 ? Math.round(100 * mosCount / totalForecasts) : 0
  const nwpSource = allForecasts.length > 0
    ? (allForecasts.some(f => (f.model ?? '').includes('neuralgcm')) ? 'NeuralGCM' : 'Open-Meteo')
    : '--'
  const hasNeuralGCM = allForecasts.some(f => (f.model ?? '').includes('neuralgcm'))
  ...
  const models = useMemo(() => {
    const set = new Set(allForecasts.map(f => f.model).filter(Boolean))
    return ['All', ...Array.from(set)]
  }, [allForecasts])
  ...
  const filtered = useMemo(() => {
    ...
    if (modelFilter !== 'All') items = items.filter(f => f.model === modelFilter)
    return items
  ...
  return (
    <div className="space-y-6">
      ...
      <PageContext id="forecasts">
        Seven-day forecasts for all 20 stations, generated by NeuralGCM (Google DeepMind's neural weather model) and corrected with an XGBoost MOS model trained on local observations. Each forecast is spatially downscaled to individual farmer GPS coordinates.
      </PageContext>
